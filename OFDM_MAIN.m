...
%=========================================================================%               
              ...
              % Demodulate only data symbols
               ...
               if SNR_DB(ss) == 25 && each_symbol==1 && each_slot==1 
                 figure;
                 subplot(2,1,1);
               ...
               if SNR_DB(ss) == 15  && each_symbol == 1 && each_slot == 1
               SNR_db=10;  
               
                 figure;
                 subplot(3,1,1);
                 time(data(200:400),'Time Response of Tx Data Bits at SNR 15 db','B');
                
                 subplot(3,1,2);
                 time(Rx_data(200:400),'Time Response of Rx Data Bits at SNR 15 db','R');
                
                 subplot(3,1,3);
                 time(data(200:400) - Rx_data(200:400),'Comparing the Tx and Rx bits at 15db','G');                   
               end
               count=count+1;
        end
...
if(choice==2)           
    image_data_receiver(Rx_data(10,:));  
    title('SNR = 15dB')
end
...
for ss=1:length(SNR_DB)
   ...
   error(ss)=length(find(xor(data(1:16000),Rx_data(ss,1:16000))))/16000;
...
% extra part Generate the result in 2.5dB, 15dB and 25dB
image_data_receiver(Rx_data(5,:));
title('SNR = 2.5dB')
image_data_receiver(Rx_data(10,:));
title('SNR = 10dB')
